...
class VRPTester():
    def __init__(self,
        ...
        self.pt_path = self.tester_params['pt_load']['path'] if self.tester_params['pt_load']['enable'] else None
    ...
    def _test_one_batch(self, episode, batch_size, clock=None):
        ...
        with torch.no_grad():
            ...
            knn_k = destroy_params['knn_k']
            if destroy_mode == 'knn-location':
                true_destroy_params = {}
                true_destroy_params['center_location'] = destroy_params['center_location'] # should be None, which means randomly choose it
                true_destroy_params['knn_k'] = [knn_k[0], min(knn_k[1], problem_size)] # for small-size instances
            ...
            for bbbb in range(iter_budget):
                ...
                destruction_mask, reduced_problem_coords, reduced_problem_demand, reduced_problem_capacity, endpoint_mask, another_endpoint, point_couples, padding_mask, new_problem_index_on_sorted_problem, \
                    problem_coords_sorted, demand_sorted= \
                            self.env.sampling_reduced_problems(destroy_mode, true_destroy_params, True)
                ...
                # logging
                escape_time,_ = clock.get_est_string(1, 1)
                self.logger.info("RECstep{},  gap:{:6f} %, Elapsed[{}], stu_l:{:6f} , opt_l:{:6f}".format(
            ...
            # return self.optimal_length.mean().item(), current_best_length.mean().item(),self.env.problem_size, gap_log
